...
contract Vehicle2 is AmountOwed {
    ...
    event Bought(uint vin, uint cost, uint fees);
    ...
    function buyVehicle() public payable validate(_cost+_fees) {
        _flag = 1;
        ...
        if (msg.sender == _owner) {
            ...
            Bought(_vin, _cost, _fees);
        }
    }
    
    function Vehicle2(uint vin, uint cost, uint fees, address buyer) public {
        _flag = 0;
        _vin = vin;
        _cost = cost;
        _fees = fees;
